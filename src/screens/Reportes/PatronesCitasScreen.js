  ...
  const loadPatronesData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        throw new Error(response.data.message || "Error al cargar patrones de citas");
      }
    } catch (err) {
    ...
    <View key={index} style={styles.horarioItem}>
      ...
      <View style={styles.horarioBar}>
        <View 
          style={[
            styles.horarioFill, 
            { width: `${(horario.cantidad / patronesData.max_citas_hora) * 100}%` }
          ]} 
        />
      </View>
  ...
  const renderEspecialidadPopular = (especialidad, index) => (
    <View key={index} style={styles.especialidadItem}>
      ...
      <View style={styles.especialidadInfo}>
        ...
        <Text style={styles.especialidadCitas}>{especialidad.total_citas} citas</Text>
      </View>
      <View style={styles.especialidadBar}>
        <View 
          style={[
            styles.especialidadFill, 
            { width: `${(especialidad.total_citas / patronesData.max_citas_especialidad) * 100}%` }
          ]} 
        />
      </View>
    </View>
  ...
  if (!patronesData) {
    return (
      <View style={styles.errorContainer}>
        <Text style={styles.errorText}>No se pudieron cargar los patrones de citas</Text>
      </View>
    );
  }
      ...
      >
        ...
        {patronesData.horarios_populares && (
          <View style={styles.section}>
            ...
            {patronesData.horarios_populares.slice(0, 8).map(renderHorarioPopular)}
          </View>
        )}
        ...
        {patronesData.especialidades_populares && (
          <View style={styles.section}>
            ...
            {patronesData.especialidades_populares.slice(0, 10).map(renderEspecialidadPopular)}
          </View>
        )}
                ...
                <View key={index} style={styles.mesItem}>
                  ...
                  <View style={styles.mesBar}>
                    <View 
                      style={[
                        styles.mesFill, 
                        { height: `${(mes.citas / patronesData.max_citas_mes) * 100}%` }
                      ]} 
                    />
                  </View>
